...
def address_to_bytes32(addr) -> str:
...
def bn_to_bytes8(value:int) -> str:
    return str("0x"+"0"*16+hex(value)[2:]).encode('utf-8')

def default_value(dict:dict, key:str, default_value):
    if key in dict:
        return dict[key]
    else:
        return default_value 

def default_enum_value(dict:dict, key:str, default_value):
    if key in dict:
...
def random_number(max_range:int) -> int:
    return current_unix_timestamp() + randint(0, max_range) + randint(0, max_range)

def extract_query(value:dict) -> str:
    query:str=""
    for i,j in value.items():
        query+="&{}={}".format(i,j)
    return query[1:]

def extract_enums(params:dict, enums:list) -> dict:
    for i in enums:
        if i in params.keys():
...
def config_logging(logging, logging_level, log_file: str = None):
    ...
    Keyword Args:
        log_file (optional): The filename to pass the logging to a file, instead of using console. Default filemode: "a"
    """
